...
<div id="accordion">
	...
	<input type='hidden' id='uploadedfilename' name='uploadedfilename' value=''>
        ...
        <input type='hidden' id='current_module' name='current_module' value='<?php if (isset($_REQUEST['__module'])) {
	...
	<span> Select your delimeter: <select name="mydelimeter">
            ...
            <option value=";">;</option>
                                    ...
                                    <input type='button' name='clearform' id='clearform' value='Clear'
                                           onclick="Reload();" class='btn btn-warning'/>
                                    <input type='submit' name='importfile' id='importfile' value='Next>>' disabled
                                           class='btn btn-primary'/>
                                </div>
                                <div class="warning" id="warning" name="warning" style="display:none"></div>
                                <!-- The container for the uploaded files -->
                                <div id="files" class="files"></div>
                                <div id="defaultpanel" align='center' style='width:100%;'><p class='msgborder'
                                                                                             style='color:green;'> You
                                        can also drag and drop files here</div>
...
<div class='mappingsection'>
    <h2>
        <div class="secondformheader">Import Data Configuration</div>
    </h2>
...
<div id='mappingheader' class='mappingheader'>
    ...
    <table style="font-size: 12px;" class="table table-striped">
        <tr>
            <td colspan='4'>
                <div align='center' style='float:right;'>
                    ...
                    <input type='hidden' id='totRecords' name='totRecords' value='<?php if (isset($records)) {
                        echo $records;
                    } ?>'/>
                    <input type='hidden' id='tmpLoc' name='tmpLoc'
                           value='<?php echo WP_CONST_ULTIMATE_CSV_IMP_DIR; ?>'/>
                    <input type='hidden' id='uploadedFile' name='uploadedFile' value="<?php if (isset($filename)) {
                        echo $filename;
                    } ?>"/>
                    ...
                    <input type='hidden' id='stepstatus' name='stepstatus' value='<?php if (isset($_REQUEST['step'])) {
                        echo $_REQUEST['step'];
                    } ?>'/>
                    <input type='hidden' id='mappingArr' name='mappingArr' value=''/>
                    <input type='button' id='prev_record' name='prev_record' class="btn btn-primary" value='<<'
                           onclick='gotoelement(this.id);'/>
                    <label style="padding-right:10px;">Change your csv row data</label>
                    <input type='button' id='next_record' name='next_record' class="btn btn-primary" value='>>'
                           onclick='gotoelement(this.id);'/>
                    Go To Row #<input type='text' id='goto_element' name='goto_element'/>
                    <input type='button' id='apply_element' name='apply_element' class="btn btn-success" value='Show'
                           onclick='gotoelement(this.id);'/>
                </div>
            </td>
        </tr>
        ...
        ?>
        <tr>
            <td class="left_align"><b>CSV HEADER</b></td>
            <td><b>WP FIELDS</b></td>
            <td><b>CSV ROW</b></td>
            <td></td>
        </tr>
        ...
        foreach ($impCE->headers as $key => $value) {
            ...
            <tr>
                ...
                <td><select name="mapping<?php print($count); ?>" id="mapping<?php print($count); ?>" class="uiButton"
                ...
                <td width="180px;">
                    <input class="customfieldtext" type="text" id="textbox<?php print($count); ?>"
                           name="textbox<?php print($count); ?>" TITLE="Replace the default value"
                           style="display: none;float:left;width:160px;" value="<?php echo $value ?>"/>
					...
					<img class="callout"
                         src="../wp-content/plugins/<?php echo WP_CONST_ULTIMATE_CSV_IMP_SLUG; ?>/images/callout.gif"/>
    ...
    <input type="hidden" id="mapping_fields_array" name="mapping_fields_array" value="<?php if (isset($mFieldsArr)) {
        print_r($mFieldsArr);
    } ?>"/>
    ...
    <div>
        <div class="goto_import_options" align=center>
            <div class="mappingactions">
                <input type='button' id='clear_mapping' class='clear_mapping btn btn-warning' name='clear_mapping'
                       value='Clear Mapping' onclick='clearMapping();' style='float:left'/>
		<span style=''>
		...
		<img src="<?php echo WP_CONST_ULTIMATE_CSV_IMP_DIR; ?>images/help.png" style="margin-top: 6px;float:right;"/>
            ...
            <div class="mappingactions">
                <input type='submit' id='goto_importer_setting' class='goto_importer_setting btn btn-info'
                       name='goto_importer_setting' value='Next >>'/>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="postbox" id="options" style=" margin-bottom:0px;">
                ...
                <div class="inside">
                    <form method="POST">
                        <ul id="settings">
                            <li>
                                <label><input name='duplicatecontent' id='duplicatecontent' type="checkbox" value="">
                                    Detect duplicate post content</label> <br>
                                <label><input name='duplicatetitle' id='duplicatetitle' type="checkbox" value=""> Detect
                                    duplicate post title</label> <br>
                                How much import per server request? <span class="mandatory">*</span> <input
                                    name="importlimit" id="importlimit" type="text" value=""
                                    onblur="check_allnumeric(this.value);">
                                <span class='msg' id='server_request_warning'
                                      style="display:none;color:red;margin-left:-10px;">You can set upto <?php if (isset($_SESSION['SMACK_MAPPING_SETTINGS_VALUES']['totRecords'])) {
                                    echo $_SESSION['SMACK_MAPPING_SETTINGS_VALUES']['totRecords'];
                                    } ?> per request.</span>
                        ...
                        <input id="startbutton" class="btn btn-primary" type="button" value="Import Now"
                               style="color: #ffffff;background:#2E9AFE;"
                               onclick="importRecordsbySettings('<?php echo site_url(); ?>');">
                        <input id="terminatenow" class="btn btn-danger btn-sm" type="button" value="Terminate Now"
                               style="" onclick="terminateProcess();"/>
                        <input class="btn btn-warning" type="button" value="Import Again" id="importagain"
                        ...
                        <div id="ajaxloader" style="display:none"><img
                                src="<?php echo WP_CONST_ULTIMATE_CSV_IMP_DIR; ?>images/ajax-loader.gif"> Processing...
                        </div>
                        <div class="clear"></div>
                    </form>
                    <div class="clear"></div>
